...
// Check if user has already booked appointment for each schedule
$booked_schedules = [];
$bookings = $database->query("SELECT scheduleid FROM appointment WHERE pid = $userid");
while ($booking = $bookings->fetch_assoc()) {
    $booked_schedules[] = $booking['scheduleid'];
}
?>
...
<head>
    ...
    <title>Faol seanslar - eDoc</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-gray-100">
    ...
    <div class="sidebar bg-white h-screen shadow-lg fixed left-0 top-0 w-64 z-10">
        <div class="p-5 border-b">
            <h1 class="text-xl font-bold text-blue-600">eDoc Patient</h1>
    ...
    <div class="main-content ml-64">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Faol seanslar/Joy olish</h1>
        ...
        <div class="card">
            <div class="card-header flex justify-between items-center">
                <h2 class="card-title">Mavjud seanslar</h2>
            ...
            <div class="card-body">
                        ...
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <div class="dashboard-card">
                                ...
                                <p class="text-gray-600 mb-1">
                                    ...
                                    <?php echo htmlspecialchars($row['docname']); ?>
                                </p>
                                <p class="text-gray-600 mb-1">
                                ...
                                <p class="text-gray-600 mb-4">
                                    ...
                                    <?php 
                                        ...
                                        $available = $row['nop'] - $appt_count;
                                        echo $available . " / " . $row['nop'] . " joy mavjud";
                                    ?>
                                </p>
                                
                                <?php if (in_array($row['scheduleid'], $booked_schedules)): ?>
                                    <button class="btn-secondary w-full cursor-not-allowed" disabled>
                                        <i class="fas fa-check mr-2"></i>Bron qilingan
                                    </button>
                                <?php elseif ($available <= 0): ?>
                                    <button class="btn-secondary w-full cursor-not-allowed" disabled>
                                        <i class="fas fa-times mr-2"></i>Joylar tugagan
                                    </button>
                                <?php else: ?>
                                    <a href="booking.php?id=<?php echo $row['scheduleid']; ?>" class="btn-primary w-full text-center">
                                        <i class="fas fa-calendar-plus mr-2"></i>Joy olish
                                    </a>
                                <?php endif; ?>
                            </div>
                        <?php endwhile; ?>
                ...
                <?php else: ?>
                    <div class="text-center py-12">
                        ...
                        <h3 class="text-xl font-semibold text-gray-700 mb-2">Mavjud seanslar topilmadi</h3>
                        <p class="text-gray-500 mb-6">
                            <?php if (!empty($search)): ?>
                                "<?php echo htmlspecialchars($search); ?>" bo'yicha hech qanday seans topilmadi.
                            <?php else: ?>
                                Hozircha mavjud seanslar mavjud emas.
                            <?php endif; ?>
                        </p>
                        <a href="doctors.php" class="btn-primary inline-block">
                            <i class="fas fa-user-md mr-2"></i>Barcha shifokorlar
                        </a>
                    </div>
                <?php endif; ?>
            </div>
